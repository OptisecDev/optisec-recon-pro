"""Scoped bounty-engagement scan for Salt Labs, a Secondary Target listed as
an in-scope asset under the Chime Bugcrowd program (lower reward tier: P1
$4500-$7000, vs. the primary Chime web-app targets). Same program, same
Engagement Rules, same "if not listed as out-of-scope, assume in scope"
clause — Salt Labs' listing under Chime's program brief is what puts these
two hosts in scope for this script, not an independent Salt Labs program.

Authorized targets (hardcoded, not configurable via CLI/env — see
ALLOWED_TARGETS / SCAN_TARGETS below). Like scripts/bounty_scan_chime.py's
Chime scope, this scope has NO wildcard entry — every host is an exact,
named target:
    - app.staging.saltlabs.com  (scanned first — staging environments are
                                  typically less hardened than production)
    - app.saltlabs.com

Reuses the project's existing scan engine, the same way bounty_scan_chime.py
does:
    ...
    - GraphQL introspection modules/vuln/graphql_probe.py (Phase 1 — see
                            ...
                            Salt Labs' wallet/rewards app is a plausible
                            GraphQL surface, so this is wired in exactly as
                            for Chime's own hosts)
    - Confidence scoring    modules/osint/confidence_engine.py
    - MITRE ATT&CK mapping  modules/osint/mitre_mapping.py

Priority ordering: app.staging.saltlabs.com is scanned before
app.saltlabs.com — staging deployments more often carry debug endpoints,
relaxed WAF rules, or stale build artifacts than the production host, so it
gets first crack at the host-level CONFIRMED-stops-the-host budget below.
SCAN_TARGETS lists the two hosts in that priority order and main() scans
strictly in that order (no cross-host concurrency), matching
bounty_scan_chime.py's sequential-scan structure.

Responsible Disclosure / PoC-only — per Chime's Bugcrowd Engagement Rules
(which govern this Secondary Target the same as the primary Chime hosts),
this scanner gathers a single confirming payload/response per finding and
enforces two stopping rules, identical to bounty_scan_chime.py's:
...
in the codebase — same categories agreed out of scope for the Chime/Verisign
engagements, carried over here because nothing in Chime's Bugcrowd brief
available to this session explicitly re-authorizes them for the Salt Labs
secondary target either. If Chime's actual program brief is later found to
explicitly permit any of these, that specific exclusion should be revisited
and this comment updated accordingly — until then, default to the narrower
Chime-equivalent scope:
...
methodology, extended here to the Salt Labs secondary target on the chance
its wallet/rewards app exposes a GraphQL endpoint): modules/vuln/
graphql_probe.py's scan_graphql() sends only standard `__schema`/`__type`
introspection queries (POST first, GET fallback) against a small fixed list
of common GraphQL paths (GRAPHQL_CANDIDATE_PATHS) — no path brute-forcing,
no mutations, no batching/aliasing/query-depth abuse. Wired into
_run_vuln_scans() alongside xss/ssrf/lfi, subject to the same
CONFIRMED-stops-the-host and one-primary-finding-per-class rules below.
...
engagement (no "*.saltlabs.com" entry exists in ALLOWED_TARGETS), which is
...
"evil-saltlabs.com" (fails: not equal to either exact hostname, and neither
hostname's wildcard branch is reachable since neither entry starts with
"*."), "saltlabs.com.evil.com" (fails: same reason — the endswith branch
that could match a suffix-confusion host is unreachable for this
ALLOWED_TARGETS set), and "staging.saltlabs.com" (fails: not equal to
"app.staging.saltlabs.com" — a bare subdomain-prefix match is never
attempted) are never mistaken for an in-scope target. Only a request whose
resolved hostname is *exactly* "app.staging.saltlabs.com" or exactly
"app.saltlabs.com" — matching left-to-right, full string — passes. See
_host_in_scope() and its accompanying tests-by-inspection in the module
docstring for scripts/bounty_scan_chime.py, whose wildcard-capable version
of this same function this one is intentionally kept compatible with (so a
future "*.saltlabs.com" scope addition would need no code change, only an
ALLOWED_TARGETS edit).
...
Usage:
    python scripts/bounty_scan_saltlabs.py
...
# Staging listed first — deliberately prioritized over production per the
# module docstring's "Priority ordering" section.
...
# Chime's program brief explicitly encourages GraphQL-endpoint testing, and
# nothing in the brief carves Salt Labs out of that encouragement. Phase 1
# (introspection-only, via modules/vuln/graphql_probe.py) IS run as part of
# this scan now — see "GraphQL introspection testing" in the module
...
def _host_in_scope(hostname: str, allowed: tuple[str, ...]) -> bool:
    ...
    Neither of this engagement's ALLOWED_TARGETS entries starts with "*." —
    both are exact hostnames — so only the `hostname == pattern` branch can
    ever match here. That makes suffix-confusion hosts like
    "evil-saltlabs.com", "saltlabs.com.evil.com", or a bare
    "staging.saltlabs.com" reliably out of scope: none of them equals
    "app.staging.saltlabs.com" or "app.saltlabs.com" exactly, and the
    wildcard/endswith branch (the only place a suffix-style match could
    occur) is unreachable for this ALLOWED_TARGETS set. Kept
    wildcard-capable (matching bounty_scan_chime.py's version of this same
    function) so a future "*.saltlabs.com" scope addition would need only
    an ALLOWED_TARGETS edit, not a code change.
    """
...
async def _run_vuln_scans(host: str, blocked: list[str]) -> dict:
    ...
    Chime's Bugcrowd Engagement Rules (which govern this Salt Labs Secondary
    Target the same as Chime's own hosts). Two stopping rules, both
    enforced here:
    ...
    1. Host-level stop: the moment any scan call returns a CONFIRMED
       ...
       this process (same rule as bounty_scan_chime.py).
...
def _build_report(started_at: str, results: dict[str, dict], blocked: list[str]) -> dict:
    return {
        "engagement": {
            ...
            "target_tier_note": (
                "Salt Labs (app.staging.saltlabs.com, app.saltlabs.com) is a "
                "Secondary Target listed as in-scope under the Chime Bugcrowd "
                "program — lower reward tier (P1 $4500-$7000) than Chime's "
                "primary web-app targets, but the same eligibility/exclusion "
                "rules apply per Chime's \"if not listed as out-of-scope, "
                "assume in scope\" clause."
            ),
            ...
            "explicitly_excluded_reason": (
                "Same categories agreed out of scope for the Chime/Verisign "
                "engagements, carried over here because nothing in Chime's "
                "Bugcrowd brief available to this session explicitly "
                "re-authorizes them for the Salt Labs secondary target either. "
...
async def main() -> None:
    ...
    out_path = REPORTS_DIR / f"bounty_scan_saltlabs_{timestamp}.json"

    with enforce_scope(blocked):
        for host in SCAN_TARGETS:
            print(f"[bounty_scan_saltlabs] scanning {host} ...", flush=True)
            ...
            print(f"[bounty_scan_saltlabs] checkpoint written to {out_path}", flush=True)

    print(flush=True)
    print(f"[bounty_scan_saltlabs] report written to {out_path}", flush=True)
    for host, data in results.items():
        confirmed = [v for v in data["vulnerabilities"] if v.get("verdict") == "CONFIRMED"]
        print(
            f"[bounty_scan_saltlabs] {host}: {len(data['vulnerabilities'])} vuln checks run, "
            f"{len(confirmed)} CONFIRMED, "
            f"{len(data['mitre_attack_path']['attack_path'])} ATT&CK kill-chain steps mapped",
            flush=True,
        )
    if blocked:
        print(f"[bounty_scan_saltlabs] WARNING: {len(blocked)} out-of-scope request(s) were blocked — see report.")
