...
def _scan_one_url(session: requests.Session, target_url: str) -> list:
    ...
    return run_concurrent_scan(file_params, lambda param: _test_param(session, parsed, params, param))


def scan_lfi(url: str, crawled_urls: list = None, crawled_forms: list = None) -> list:
    ...
    every page reachable from `url`, not just `url` itself (a bare `url`
    with no query string tested nothing before this). Defaults to None so
    existing single-URL callers are unaffected. `crawled_forms` is accepted
    for interface parity with the other scan_*() functions but unused —
    LFI here only ever tests URL query params, never form fields."""
